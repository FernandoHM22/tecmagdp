			...
			<tbody>
				...
				if (mysqli_num_rows($SearchReddin) > 0) { 
					while ($datosReddin = mysqli_fetch_array($SearchReddin)){
						...
						?>
						<tr>
							<td style="text-align: left; font-size: 13px;"><?=$datosReddin["oportunidadConsenso"]?></td>
							<td style="text-align:justify; font-size: 13px;"><?=$datosReddin["porque_mejora"]?></td>
							<td style="text-align: center; font-size: 13px;"><?=$datosReddin["fecha"]?></td>
							<td>
								<input type="text" name="id" hidden value="<?php echo $idPlan ?>">
								<input type="text" name="estatus" value="Actual" hidden> 
