    ...
    <table class="table table-responsive-md table-bordered tablaObjetivos">
        <thead>
            <tr>
               ...
               <th width="10%" style="text-align:center; vertical-align: middle;font-size:14px;">Acciones</th>
           </tr>
       </thead>
       <tbody>
           <?php

           $buscarInfo = mysqli_query($conn, "SELECT * FROM objetives_gdp where obj_no_reloj = '$no_reloj' AND año_reg LIKE '$año' AND resultadoFinal == 0 ORDER BY categoria ASC");
           if (mysqli_num_rows($buscarInfo) > 0) {
               ...
               while ($datos = mysqli_fetch_array($buscarInfo)){ 
                ...
                <td><?php if($datos["archivoEvidencia"] != ""){?><a href="<?= $datos["archivoEvidencia"];?>" download><i class="fas fa-file-alt"></i></a><?php } ?></td>
